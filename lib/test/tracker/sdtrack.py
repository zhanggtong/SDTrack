...
class SDTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=False)#改
        vocab_path = '/home/zhanggt/SDTrack/pretrained_models/bert/bert-base-uncased-vocab.txt'
        self.k = 1
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(
                template=self.z_dict1.tensors, search=x_dict.tensors, text=self.text_input, training=False, tgt_pre=self.tgt_all,
                reverse_pre=self.reverse_all, nlp = self.text,path = info['path'])
            self.tgt_all = out_dict['tgt']
            self.reverse_all = out_dict['reverse']
            #print(len(self.tgt_all), 'self.tgt shape')            
