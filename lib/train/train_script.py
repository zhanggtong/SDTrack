...
from lib.train.actors import SDTrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "sdtrack":
        net = build_sdtrack(cfg)
    else:
    ...
    if settings.script_name == "sdtrack":
        ...
        actor = SDTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
