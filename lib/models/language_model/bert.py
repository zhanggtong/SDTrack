...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, prompt=None, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        #embedding_output = torch.ones(1,41,768).to('cuda')
...
class BERT(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor, prompt=None):
        ...
        if self.enc_num > 0:
            all_encoder_layers = self.bert(tensor_list.tensors, prompt, token_type_ids=None, attention_mask=tensor_list.mask)
