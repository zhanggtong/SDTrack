...
from .file_utils import WEIGHTS_NAME, CONFIG_NAME, cached_path
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, prompt=None ,token_type_ids=None, attention_mask=None,
        ...
        prompt_mask = torch.ones(1, 1).to('cuda')
...
class BERT(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor, prompt=None):

        if self.enc_num > 0:
            all_encoder_layers, layer_atts = self.bert(tensor_list.tensors, prompt, token_type_ids=None, attention_mask=tensor_list.mask)
            # use the output of the X-th transformer encoder layers
            xs = all_encoder_layers[self.enc_num]
        else:
        ...
        return xs, all_encoder_layers, layer_atts

class TinyBERT(nn.Module):
    def __init__(self, path: str,train_bert: bool, continue_train:bool,fit_size=768,hidden_size = 312,enc_num = 4):
        super().__init__()
        if continue_train:
            self.tinybert = BertModel.from_pretrained(path)
        self.tinybert = BertModel.from_scratch(path)
        self.enc_num = enc_num
    ...
    def forward(self, tensor_list: NestedTensor, prompt=None):
        ...
        if self.enc_num > 0:
            all_encoder_layers, layer_atts= self.tinybert(tensor_list.tensors, prompt, token_type_ids=None, attention_mask=tensor_list.mask)
            # use the output of the X-th transformer encoder layers
            # xs = all_encoder_layers[self.enc_num - 1]
            tmp = []
            for s_id, sequence_layer in enumerate(all_encoder_layers):
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, prompt=None, token_type_ids=None, attention_mask=None,
        ...
        sequence_output, pooled_output = self.bert(input_ids, prompt,token_type_ids, attention_mask,
