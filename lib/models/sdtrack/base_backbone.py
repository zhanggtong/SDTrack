...
from lib.models.sdtrack.utils import combine_tokens, recover_tokens
...
class BaseBackbone(nn.Module):
    ...
    def forward_features(self, z, x, text, mask=None):
        ...
        # aux_dict = {"attn": None}
        final_cross_attn = cross_attn_list[-1] if cross_attn_list else None
        aux_dict = {"attn": final_cross_attn}  # 存入aux_dict返回
        x = self.norm_(x)

        return x, aux_dict

    def patch(self, z, x, mask=None):
        B = x.shape[0]
