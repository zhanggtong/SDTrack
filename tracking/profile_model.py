...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='sdtrack', choices=['sdtrack'],
...
def evaluate_vit(model, template, search, text, reverse, tgt):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search, text, False, reverse, tgt),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search, text, False, reverse, tgt)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search, text, False, reverse, tgt)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "sdtrack":
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_sdtrack
        model = model_constructor(cfg, training=False)
        # get the template and search
        template = torch.randn(bs, 3, z_sz, z_sz)
        search = torch.randn(bs, 3, x_sz, x_sz)
        text = torch.randint(0, 40, size=(1, 40))
        text_mask = torch.randint(0, 40, size=(1, 40))
        # text = torch.randint(0, 64, size=(1, 64))
        # text_mask = torch.randint(0, 64, size=(1, 64))
        text = NestedTensor(text.to(device), text_mask.to(device))
        reverse = []
        tgt = []
        # transfer to device
        ...
        if merge_layer <= 0:
            evaluate_vit(model, template, search, text, reverse, tgt)
        else:
