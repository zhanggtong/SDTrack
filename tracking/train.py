...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='sdtrack', help='training script name')
    parser.add_argument('--config', type=str, default='hivitb_224', help='yaml configure file name')
    parser.add_argument('--save_dir', type=str, default='./hivitb_224', help='root directory to save checkpoints, logs, and tensorboard')
    ...
    parser.add_argument('--nproc_per_node', default=2, type=int, help="number of GPUs per node")  # specify when mode is multiple
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "/home/muyh/anaconda3/envs/sd/bin/python lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
        ...
        train_cmd = "OMP_NUM_THREADS=2 /home/zhanggt/anaconda3/envs/ostrack/bin/python -m torch.distributed.run --nproc_per_node %d --master_port %d lib/train/run_training.py " \
        ...
        train_cmd = "/home/muyh/anaconda3/envs/tlot/bin/python -m torch.distributed.run --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
